...
export const AdminCatalogView = () => {
    ...
    const handleBulkImportManual = () => {
        ...
        setTimeout(() => {
            ...
            const items = lines.map(line => {
                ...
                const fullName = nameBase.toUpperCase();
                const price = parseFloat(parts[5] || parts[parts.length - 1]?.replace(/[^0-9.]/g, '')) || 0;
                
                // --- DETECÇÃO DE DUPLICADOS (CATÁLOGO MESTRE) ---
                // Verifica no cache local de produtos carregados
                const isDuplicate = products.some(p => areProductNamesSimilar(p.name, fullName));
    ...
    const handleSaveBulk = async () => {
        setLoading(true);
        // Filtra para remover duplicados antes de enviar ao banco, se houver
        const itemsToSave = processedItems.map(it => ({
            name: it.name,
        ...
        if(success) {
            playSound('cash');
            setToast({msg: `${itemsToSave.length} itens injetados!`, type: 'success'});
