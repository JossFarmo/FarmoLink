...
export const AdminSupportView = ({ user }: { user: User }) => {
    ...
    // Scroll inteligente: Apenas dentro do container, sem mover a janela/página
    useEffect(() => {
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20 h-full max-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-3xl border border-gray-100 shadow-sm gap-4 shrink-0">
                <div>
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-xl">
                        {['OPEN', 'RESOLVED', 'ALL'].map(s => (
                            <button 
                                ...
                                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${filterStatus === s ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-400'}`}
                            >
                                {s === 'OPEN' ? 'Abertos' : s === 'RESOLVED' ? 'Resolvidos' : 'Todos'}
                            </button>
                        ))}
                    </div>
                    <button onClick={loadTickets} className="p-3 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all">
                    ...
                    {filtered.map(ticket => (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black ${ticket.status === 'OPEN' ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400'}`}>
                                    {ticket.user_name?.charAt(0) || 'U'}
                                </div>
                                <div className="min-w-0">
                                    <h4 className="font-bold text-gray-800 text-sm truncate max-w-[120px]">{ticket.subject}</h4>
                                    <p className="text-[10px] text-gray-400 font-bold uppercase">{ticket.user_name}</p>
                                </div>
                            </div>
                            <Badge color={ticket.status === 'OPEN' ? 'green' : 'gray'}>{ticket.status}</Badge>
                        </div>
                    ))}
                </div>
                ...
                <div className="lg:col-span-2 relative h-full">
                    {selectedTicket ? (
                        <Card className="h-full p-0 flex flex-col border-0 shadow-2xl overflow-hidden animate-scale-in">
                            {/* Header do Chat - FIXO */}
                            <div className="p-6 bg-gray-50 border-b flex justify-between items-center shrink-0 z-10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-black text-gray-800 text-sm">{selectedTicket.subject}</h3>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase">{selectedTicket.user_email}</p>
                                    </div>
                                </div>
                                <div className="flex gap-2 items-center">
                                    {selectedTicket.status === 'OPEN' && (
                                        <Button 
                                            ...
                                            className="text-[10px] py-2 px-4 bg-emerald-600 hover:bg-emerald-700 shadow-md"
                                        >
                                            {actionLoading ? <Loader2 size={12} className="animate-spin"/> : 'Encerrar Problema'}
                                        </Button>
                                    )}
                                    <button 
                            ...
                            <div 
                                ...
                                className="flex-1 overflow-y-auto p-8 space-y-4 bg-white custom-scrollbar"
                            >
                                {messages.map((m, idx) => (
                            ...
                            {selectedTicket.status === 'OPEN' ? (
                                <div className="p-6 bg-gray-50 border-t flex gap-4 shrink-0">
                                    <input 
                                        className="flex-1 p-4 bg-white border rounded-2xl outline-none focus:ring-2 focus:ring-emerald-500 transition-all text-sm shadow-inner"
                                    ...
                                    <button 
                                        ...
                                        className="p-4 bg-emerald-600 text-white rounded-2xl hover:bg-emerald-700 shadow-xl shadow-emerald-500/20 transition-all active:scale-95"
                                    >
                                        <Send size={24}/>
                                    </button>
                                </div>
